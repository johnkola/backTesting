...
ML strategies opt in to filesystem caching by implementing `PersistableModelStrategy`. Before calling `strategy.initialize`, `BacktestEngine` checks for that interface and hands the strategy a `ModelContext` carrying `(instrumentId, sourceId, timeframe, ModelLoadPolicy, ModelStore, FeatureStore, pinnedVersionId)`. The strategy is responsible for computing a deterministic cache key (`ModelStore.computeCacheKey`), trying `store.load(strategyName, key)` (or the 3-arg pinned overload when `pinnedVersionId != null`), and acting on the policy: `LOAD_OR_TRAIN` falls through to a train + `store.save(...)` on miss, `TRAIN_FRESH` skips the lookup entirely, and `LOAD_ONLY` throws `ModelNotCachedException` rather than training. The training step writes the network, the fitted normalizer, and a JSON metadata sidecar in one call.

`ModelStore` is a thin wrapper around `data/models/<strategy>/<key>/<versionId>/`. There is no DB-backed model registry yet — the cache key is part of the path and `metadata.json` is debug info plus a DL4J version stamp used to reject models trained against a different runtime. The `BacktestEngine.train` entry point shares its candle-load + series-build prep with `run` (see `BacktestEngine.prepare`) so a `train` invocation goes through the same data path as the equivalent `run`; only the post-`initialize` steps differ (`train` returns a `TrainingSummary`, `run` continues into the bar loop). See the README "Model cache" section for the user-facing contract.

**Versioning.** Each `ModelStore.save()` writes to a fresh `<versionId>` subdir (compact UTC timestamp from `ModelStore.VERSION_FORMAT`, e.g. `20260511T134522.123Z` — filesystem-safe on Windows + Linux + macOS, lexicographically sortable, millisecond-precision so realistic train cadences can't collide). `load(strategy, key)` returns the lexicographically-latest version subdir, which is "current" for default `run` purposes. Two fallbacks make the layout transition seamless: (a) the 2-arg `load` reads legacy flat-layout entries (files directly under `<key>/`) when no version subdirs exist; (b) it skips subdirs that don't match `VERSION_PATTERN` so stray dirs don't poison resolution. The web `/api/models` walker mirrors the same precedence — versioned subdirs preferred, flat layout as fallback — and emits one row per version so the Models page can show history.

**Version pinning.** `ModelStore.load(strategy, key, versionId)` is the pin-aware overload: a non-null `versionId` consults only `<keyDir>/<versionId>/` and **never** falls back to the legacy flat layout (pre-versioning entries have no id to match). The `run` CLI exposes this as `--model-version <id>` (`BacktestCommand` → `BacktestEngine.run(..., policy, pinnedVersionId)` → `ModelContext`); `train` deliberately does not — training always writes a fresh version. On miss the strategy throws `ModelNotCachedException` carrying the pinned id, and the CLI switches its hint from "run `train …`" to "see /api/models for available versions".

**Retention (`keep-last-N`).** `ModelStore` accepts a `keepLastN` at construction (0 = disabled). When positive, `save()` calls `pruneTo(strategy, key, keepLastN)` after the write completes, deleting all but the N lexicographically-latest version subdirs under that key. Pruning is opportunistic: a failure (e.g. a file lock) is logged and swallowed, the save itself never fails on retention. Only `VERSION_PATTERN`-matching subdirs are eligible — legacy flat-layout entries and stray subdirs are left alone. The retention number flows from `application.properties` (`model.retention.keepLastN`, default 5) through `AppConfig`, optionally overridden by `train --keep-last <N>`; `TrainCommand` builds the `ModelStore` explicitly with this value and hands it to `BacktestEngine` via the 6-arg constructor. The default-stores engine constructor still uses `keepLastN=0` (no retention) so tests and ad-hoc engine users keep the unbounded-history behavior they had before. `pruneTo` is `public` so operator tooling (or a future `prune-models` CLI) can invoke it directly.
...
- `candles_weekly` / `candles_monthly` — TimescaleDB continuous aggregates over `candles WHERE timeframe='D1'`. Bucket via `time_bucket('7 days' / '1 month', timestamp)`, aggregate with `FIRST(open) / MAX(high) / MIN(low) / LAST(close) / SUM(volume)`. Refresh policies (hourly for W1, every 12h for M1) keep them current incrementally. No Java or web code reads them yet — they exist so a future multi-timeframe consumer can query already-aggregated rows instead of re-aggregating client-side. Created `WITH NO DATA` so schema bootstrap doesn't block on backfilling the materialization.
