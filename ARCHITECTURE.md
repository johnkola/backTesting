...
**Version pinning.** `run --model-version <id>` is wired end-to-end. `BacktestCommand` sets `ModelContext.pinnedVersionId` with policy `LOAD_ONLY`; `NeuralNetworkStrategy.buildTrainBody` forwards it as `version_id` on `/api/nn/train`, and the loader returns that exact version under the resolved cache key (or 404 if it isn't on disk → `ModelNotCachedException`, which the CLI turns into a "see /api/models" hint). The resolved version then pins the `/api/nn/predict_range` call. A pin only matches a version stored under the cache key the run *computes*, so changing the data range or hyperparameters (which change the key) reports the pin as missing — same semantics as the old Java `ModelStore` pin. The loader never trains or falls back to "latest" to satisfy a pin.
